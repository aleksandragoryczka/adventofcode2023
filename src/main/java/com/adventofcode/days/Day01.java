...
public class Day01 {
    public static void task1(String filePath) {
        ...
        int calibrationValuesSum = cleanedLines.stream()
                .map(line -> String.valueOf(line.charAt(0)) + String.valueOf(line.charAt(line.length() - 1)))
    ...
    public static void task2(String filePath) {
        List<String> lines = ReaderUtil.readLineByLineToList(filePath);
        List<String> wordDigitsList = Arrays.asList("one", "two", "three", "four", "five", "six", "seven", "eight",
                "nine");
