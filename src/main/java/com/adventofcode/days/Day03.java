...
public class Day03 {
    ...
    public static void task2(String filePath) {
            ...
            while (j < charsMatrix.get(i).size()) {
                ...
                if (digitsBuilder.length() > 0) {
                    ...
                    if (gearsCoordinates != null) {
                        gearsCoordinatesMap = updateGearsMap(gearsCoordinatesMap, gearsCoordinates,
                                Integer.parseInt(digitsBuilder.toString()));
                    }
                    digitsBuilder = new StringBuilder();
                }
                j++;
        ...
        int gearsRatiosSum = gearsCoordinatesMap.values().stream().filter(values -> values.size() == 2)
                .map(values -> values.get(0) * values.get(1)).mapToInt(Integer::intValue).sum();
    ...
    private static boolean checkIsAdjacent(int i, int j, int stringLength, List<List<Character>> charsMatrix) {
        if (i == 0) {
            if (j - stringLength == 0) {
                for (int x = 0; x < i + 2; x++) {
                    if (iterateHelper(0, stringLength + 1, charsMatrix, x))
                        return true;
                }
            } else if (j == charsMatrix.get(i).size()) {
                for (int x = 0; x < i + 2; x++) {
                    if (iterateHelper(j - stringLength - 1, j - 1, charsMatrix, x))
                        return true;
                }
            } else {
                for (int x = 0; x < i + 2; x++) {
    ...
    private static HashMap<List<Integer>, List<Integer>> updateGearsMap(
            HashMap<List<Integer>, List<Integer>> gearsCoordinatesMap, List<Integer> gearsCoordinates,
            Integer partNumber) {
        if (gearsCoordinatesMap.containsKey(gearsCoordinates)) {
            gearsCoordinatesMap.put(gearsCoordinates,
                    Arrays.asList(partNumber, gearsCoordinatesMap.get(gearsCoordinates).get(0)));
        } else {
    ...
    private static List<Integer> iterateHelperFindGear(int startCondition, int endCondition,
            List<List<Character>> charsMatrix, int verticalindex) {
    ...
    private static boolean iterateHelper(int startCondition, int endCondition, List<List<Character>> charsMatrix,
            int verticalindex) {
        String regex = "[^\\d.]";
        for (int i = startCondition; i < endCondition; i++) {
